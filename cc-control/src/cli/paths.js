...
/**
 * 解析所有关键路径
 * 当前文件位于 cc-control/src/cli/，以此为基准解析
 *
 * cc-control/（projectRoot）
 *   src/awf.js              ← CLI 入口
 *   src/server/server.js     ← HTTP Session server
 *   scripts/bootstrap.sh     ← tmux session 启动脚本
 *   src/prompts/run/         ← 阶段提示词模板
 *   plugin/commands/         ← slash commands（Claude Code 自动发现）
 *   plugin/skills/           ← skills（Claude Code 自动发现）
 */
/**
 * 插件命名空间前缀
...
export function getPaths() {
  ...
  return {
    ...
    prompts: path.join(projectRoot, 'src', 'prompts', 'run'),               // 阶段提示词模板
