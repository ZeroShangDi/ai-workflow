...
async function initWorkspace(paths, force, version) {
  ...
  const ensureSkeleton = async () => {
    // 目录结构（含动态规划 proposal/事件的稳定扩展边界）
    const dirs = ['bugs', 'issues', 'decisions', 'dynamic-planning/proposals', 'context', 'logs', 'reports/lint', 'reports/test', 'reports/review', 'reports/perf', 'reports/summary', 'versions'];
