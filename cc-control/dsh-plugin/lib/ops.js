...
export function createOps({ ctx, config = {}, log = () => {}, onEvent = () => {}, loadMcpClient = loadMcpClientReal, loadAgent = loadAgentBits, loadLlm = loadLlmBits }) {
  ...
  async function createSession({ cwd, mountMcp = true } = {}) {
    ...
    try {
      ...
      const sessionId = agent?.session?.header?.id ?? agent?.id ?? null;
      if (!sessionId) return { ok: false, error: 'agents.create 未回可用的会话 id' };
      createdByAwf.add(sessionId);
      return { ok: true, sessionId, agent, mounted };
    } catch (err) {
  ...
  const ops = {
    ...
    async 'session.create'(args = {}) {
      ...
      return {
        ok: true,
        result: { sessionId: created.sessionId, agentPreset: config.agentPreset ?? 'default', cwd, mcp: { mounted: created.mounted, toolsAtCreate: null } },
      };
    },
    ...
    async 'plan.launch'(args = {}) {
      ...
      try {
        ...
        return {
          ok: true,
          result: { sessionId, accepted: r?.accepted === true, url: sessionUrl(sessionId), mcp: { mounted: created.mounted } },
        };
      } catch (err) {
