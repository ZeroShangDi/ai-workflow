...
function NodeRow({
  ...
  return (<>
    <li className={`node ${status}`} style={{
    ...
    {node.children.length > 0 && node.children.map(c => <NodeRow key={c.id} node={c} depth={depth + 1} />)}
  </>);
}
export default function WbsTree({
  ...
  const refresh = useCallback(async () => {
    const st = await client.get(API.state).catch(() => null);
    if (st && Array.isArray(st.wbs)) setModel(buildWbsTree({
      wbs: st.wbs,
      tasks: st.tasks || []
    }));
  }, [client]);
  ...
  return (<main className="wbs-tree">
    <header>
      <h1>WBS Tree</h1>
      <span>{stats.total} 项 · {stats.withTask} 个任务</span>
      {chips.map(k => <span key={k} className={`chip ${k}`}>{k} {stats.byStatus[k]}</span>)}
    </header>
    {model.roots.length === 0 ? <p className="empty">尚未规划（无 WBS 数据）。请先执行 awf plan。</p> : <ul className="tree">
