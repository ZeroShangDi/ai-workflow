...
export function useDecisionsPage({
  ...
  const id = item?.id;
  const approvable = item?.subject?.capability === 'dynamic_planning' && item.status === 'awaiting_human';
  ...
  return {
    ...
    id,
    approvable,
    alternative: () => operation.action(API.proposalAction(item.subject.proposal_id, 'alternative'), { reviewer, instruction }),
    overrideable,
    ...
    approve: () => operation.action(API.resolveDecision(id), {
      reviewer: reviewer.trim(),
      note: instruction.trim(),
      outcome: 'approve'
    }),
    override: () => operation.action(API.overrideDecision(id), {
