...
export default function Diagnostics({ sid, project } = {}) {
  ...
  const refresh = useCallback(async () => {
    const [metrics, diagnosis] = await Promise.all([
      client.get(API.metrics).catch(() => null),
      client.get(API.diagnostics).catch(() => null),
    ]);
    setModel(
      toDiagnosticsModel({
        metrics,
    ...
    <main className="diagnostics">
      ...
      <section className="summary">
        ...
        <div className="card">
          ...
          <div className="value">
            {model.throughput != null ? `${model.throughput.toFixed(2)} tok/s` : '—'}
          </div>
        ...
        <div className="card">
          ...
          <div className="meta">
            {model.activeAgents != null ? `${model.activeAgents} 个 Agent 运行中` : '—'}
          </div>
        </div>
        <div className="card">
          <div className="label">上下文</div>
          <div className="value">
            {model.context ? `${model.context.usedPercentage ?? '—'}%` : '—'}
          </div>
      ...
      <section className="panels">
        ...
        <div className="card">
          ...
          <dl>
            ...
            <dt>运行中 Agent</dt>
            <dd>
              {model.maxAgents != null ? `${model.activeAgents ?? 0}/${model.maxAgents}` : '—'}
            </dd>
          </dl>
        </div>
      </section>
