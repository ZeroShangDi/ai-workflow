 ...
 */
export function toDiagnosticsModel({
  metrics,
  diagnosis
} = {}) {
  ...
  const throughput = sp.currentTokensPerSecond !== undefined ? num(sp.currentTokensPerSecond) : sp.averageTokensPerSecond !== undefined ? num(sp.averageTokensPerSecond) : null;
  const throughputBasis = sp.basis === 'recent_60s' ? '最近 60s 端到端吞吐' : sp.basis === 'average' ? '全程端到端吞吐' : null;
  const rec = diagnosis && typeof diagnosis.diagnosis === 'object' && diagnosis.diagnosis !== null ? diagnosis.diagnosis : null;
