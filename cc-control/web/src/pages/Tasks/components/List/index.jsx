...
export default function TaskList({
  ...
  const statusOptions = [
    ...TASK_STATUSES,
    ...Object.keys(statusCounts).filter(s => !TASK_STATUSES.includes(s)),
  ];
  const sourceOptions = [
    ...TASK_SOURCES,
    ...Object.keys(sourceCounts).filter(s => !TASK_SOURCES.includes(s)),
  ];
  ...
  return (
    <>
      <header className="pane-header">
        ...
        <div className="pane-tools">
          ...
          <Select
            ...
            {sourceOptions.map(s => (
              <option key={s} value={s}>
                {sourceShort(s)} ({sourceCounts[s] || 0})
              </option>
            ))}
          </Select>
          ...
          <Button
            ...
            aria-label={panel === 'overview' ? '切换到任务详情' : '切换到运行概览'}
      ...
      <div className="record-list task-list">
        ...
        {!visible.length && <Empty>{tasks.length ? '没有匹配的任务' : '暂无任务'}</Empty>}
      </div>
      {!!message && (
