...
import { TASK_ORIGINS, originOf, originShort, originLabel } from '../../model.js';
export default function TaskList({
  ...
  const presentOrigins = new Set(tasks.map(originOf));
  // 只有一种来源时这个筛选没有意义，直接不显示 —— 避免过渡期多一个空控件。
  const originOptions = presentOrigins.size > 1 ? [...TASK_ORIGINS.filter(o => presentOrigins.has(o)), ...[...presentOrigins].filter(o => !TASK_ORIGINS.includes(o))] : [];
  return (<><header className="pane-header">
      ...
      <div className="pane-tools">
        ...
        {!!originOptions.length && <Select aria-label="任务来源" value={originFilter} onChange={e => setOriginFilter(e.target.value)}>
          <option value="all">全部来源</option>
          {originOptions.map(o => <option key={o} value={o}>{originShort(o)}</option>)}
        </Select>}
      </div>
      ...
      return (<div key={t.id} role="button" tabIndex={0} className={`record-row ${task?.id === t.id ? 'selected' : ''}`} onClick={() => setSelected(t.id)} onKeyDown={e => {
        ...
        <span className={`record-id status-${t.status}${origin === 'plan' ? '' : ' record-id-derived'}`} title={origin === 'plan' ? undefined : originLabel(origin)}>{t.id}</span>
