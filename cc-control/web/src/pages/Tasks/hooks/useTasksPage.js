...
export function useTasksPage(data) {
  ...
  const visible = tasks.filter(t => (filter === 'all' || t.status === filter) && (originFilter === 'all' || originOf(t) === originFilter) && display(t).toLowerCase().includes(search.toLowerCase()));
