  ...
  return (
    <div className="workflow-page">
      <header>
        ...
        <p className="muted">
          这个页面是接入要求里先建的<strong>空页面</strong>：入口已接通、数据与动作仍由 AWF 的 API 提供，
          完整业务 UI 待逐页指导后实现。
        </p>
      </header>
      <section>
        <h3>当前状态</h3>
        <p role="status">
          {project ? `已选择项目：${project}` : '还没有选择项目 —— 用左侧栏底部的「添加项目」选择工作目录后再看本项目的数据。'}
        </p>
      ...
      <section>
        ...
        <div className="actions">
          {LINKS.map(([key, label]) => (
            <button key={key} type="button" onClick={() => setView?.(key)}>{label}</button>
          ))}
        </div>
      </section>
    </div>
