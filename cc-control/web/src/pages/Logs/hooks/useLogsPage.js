...
export function useLogsPage(props) {
  ...
  const filtered = events.filter(e => (!runId || e.runId === runId) && e.seq > clearedSeq && display(e).toLowerCase().includes(search.toLowerCase()));
  const text = source === 'session' ? (data.status?.snapshot || '').split('\n').filter(line => line.toLowerCase().includes(search.toLowerCase())).join('\n') : filtered.map(e => `${formatTime(e.at || e.ts)}  ${e.type}  ${JSON.stringify(e.payload || {})}`).join('\n');
  useEffect(() => {
