...
export function matrixCells(tasks, limit = 16) {
  ...
  const head = start
    ? [{ id: 'completed-group', status: 'done', title: `${start} 个已完成任务`, count: start }]
    : [];
  const tail =
    end < tasks.length
      ? [
          {
            id: 'pending-group',
            status: 'pending',
            title: `${tasks.length - end} 个待执行任务`,
            count: tasks.length - end,
          },
        ]
      : [];
  ...
  return [
    ...head,
    ...middle.slice(0, middle.length > capacity ? capacity - 1 : capacity),
    ...(middle.length > capacity
      ? [
          {
            id: 'other-group',
            status: 'mixed',
            count: middle.length - capacity + 1,
            title: '其余任务，请在任务页查看',
          },
        ]
      : []),
