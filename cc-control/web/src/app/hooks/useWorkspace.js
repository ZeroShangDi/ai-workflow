...
export function useWorkspace(project, view = 'run') {
  ...
  const [events, setEvents] = useState([]), [revision, setRevision] = useState(0);
  ...
  usePolling(async signal => {
    ...
    await Promise.all(Object.entries(paths).map(async ([key, path]) => {
      ...
      } catch (error) { if (!signal.aborted) setErrors(old => ({ ...old, [key]: error.message })); }
    }));
  ...
  usePolling(async signal => {
    try {
      ...
      for (let page = 0; page < 8; page++) {
        ...
        if (response.tailSeq < cursor.current) {
          cursor.current = 0; setEvents([]); setEventNotice('运行服务已重启，重新读取当前事件。');
          continue;
        }
    ...
    } catch (error) { if (!signal.aborted) setErrors(old => ({ ...old, events: error.message })); }
