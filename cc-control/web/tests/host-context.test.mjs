...
test('DSH identity stays separate from project path and run identity', () => {
  ...
  const url = new URL(
    routeUrl(
      { view: 'run', project: '/project', runId: '' },
      'http://localhost/?mode=dsh&pid=workspace-1&sid=session-2',
    ),
    'http://localhost',
  );
...
test('host context requires matching parent, origin and active session', () => {
  ...
  const event = {
    source: parent,
    origin,
    data: { type: 'awf:context', ...context, projectRoot: '/my project' },
  };
  ...
  assert.equal(
    acceptHostMessage({ ...event, data: { ...event.data, sid: 'old-session' } }, options),
    null,
  );
});
...
test('CC standalone supports legacy paths and explicit project IDs', () => {
  ...
  assert.equal(
    resolveProject(readHostContext({ search: '?mode=cc&pid=p1&sid=s1' }), [
      { projectId: 'p1', projectRoot: '/repo' },
    ]),
    '/repo',
  );
  assert.equal(
    resolveProject(readHostContext({ search: '?pid=unknown' }), [{ projectRoot: '/boot' }]),
    null,
  );
});
