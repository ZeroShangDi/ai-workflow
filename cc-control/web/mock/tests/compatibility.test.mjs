...
test('all read endpoints are implemented; unknown routes cannot reach a live server', () => {
  ...
  for (const key of ['status','snapshot','state','runs','events','decisions','proposals','metrics','diagnostics']) assert.equal(server.handle(url(API[key])).status, 200, key);
  assert.equal(server.handle(url('/unknown')).status, 404);
});
test('pause prevents progression, resume advances tasks and cursor events', () => {
  const s = createMockServer(); const get = path => s.handle(url(path)).body;
  const post = body => s.handle(url(API.workflowMode), { method: 'POST', body });
  post({ mode: 'pause' }); const before = get(API.state).tasks;
  for (let i=0;i<3;i++) s.advance();
  assert.deepEqual(get(API.state).tasks, before);
  post({ mode: 'run' }); for (let i=0;i<3;i++) s.advance();
  assert.ok(get(API.state).tasks.filter(t=>t.status==='done').length > before.filter(t=>t.status==='done').length);
...
test('approval validates reviewer and updates proposal and tasks', () => {
  const s = createMockServer(); const path = url(API.approveProposal('P-001'));
...
test('navigation preserves preview parameters and clears old session routing', () => {
  const path = routeUrl({view:'tasks',project:'/a b',runId:'R1'},'http://mock/?mock=1&scenario=empty&sid=old');
  const result = new URL(path,'http://mock');
  assert.equal(result.searchParams.get('mock'),'1'); assert.equal(result.searchParams.has('sid'),false);
...
test('transport scopes requests and normalizes HTTP errors', async () => {
  const client = createApiClient({base:'http://mock',project:'/safe',httpFetch: async input => { assert.equal(new URL(input).searchParams.get('p'),'/safe'); return new Response('{"error":"conflict"}',{status:409}); }});
  assert.deepEqual(await client.post('/send?p=other',{text:'hello'}),{ok:false,error:'conflict'});
});
test('remaining writes: start, reply, interrupt, override, resolve, diagnosis', () => {
  const s = createMockServer({scenario:'idle'}), post=(server,path,body={})=>server.handle(url(path),{method:'POST',body});
  ...
  assert.equal(post(s,API.resolveDecision('D-010'),{reviewer:'test',outcome:'approve'}).body.ok,true);
  assert.equal(post(s,API.diagnostics).status,202);s.advance();
...
test('timeout aborts transport; writes are not retried', async()=>{
  let calls=0;
  const client=createApiClient({httpFetch:(_, {signal})=>new Promise((resolve,reject)=>{calls++;signal.addEventListener('abort',()=>reject(signal.reason));})});
  await assert.rejects(client.post('/send',{text:'x'},{timeoutMs:10}),/超时/);assert.equal(calls,1);
});
