...
export interface Plan {
  status: 'empty' | 'generating' | 'ready' | 'approved' | 'failed';
  version: number;
  summary: string;
  tasks: Task[];
  error?: string | null;
...
export interface Environment {
  status: 'unread' | 'reading' | 'ready' | 'failed';
  files: { path: string; status: string; summary: string }[];
  branch: string;
  runtime: string;
  warnings: string[];
  error?: string | null;
...
export interface Run {
  ...
  startedAt: string;
  endedAt?: string;
  counts: Record<'total' | TaskStatus, number>;
}
...
export interface Proposal {
  ...
  operations: {
    ...
    relation: { type: 'prerequisite_for'; targetTaskId: string };
  }[];
  analysis: { affectedTaskIds: string[]; decisionReasons?: string[] };
  decision?: { decisionId: string; runStamp: string };
  approvedBy?: string;
  reviewedBy?: string;
  alternative?: string;
}
export interface DecisionEvent {
  event: 'decision_requested' | 'decision_completed' | 'decision_overridden';
  decision_id: string;
  runStamp: string;
  status?: 'pending_review' | 'reviewed' | 'awaiting_human';
  created_at?: string;
  at?: string;
  instruction?: string;
...
export interface ReadResponses {
  '/projects/directories': Result<{
    directories: { path: string; name: string; existing: boolean }[];
  }>;
  '/workspace': Result<{
    projectRoot: string;
