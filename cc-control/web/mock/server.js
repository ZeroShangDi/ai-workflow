...
export function createMockServer({ scenario = 'demo' } = {}) {
  ...
  function applyProposal(project, proposal, body) {
    ...
    if (scenario === 'conflict' && !proposal.retryCount) { proposal.status = 'conflicted'; emit(project, 'dynamic_planning.conflicted', { proposalId: proposal.proposalId }); return null; }
    for (const operation of proposal.operations) {
      const target = operation.relation?.targetTaskId;
      if (target && !project.state.tasks.some(t => t.id === target)) return fail('目标任务不存在');
    }
    for (const operation of proposal.operations) {
      if (operation.type === 'insert_task' && !project.state.tasks.some(t => t.id === operation.task.id)) project.state.tasks.push({ ...operation.task, deps: [], status: 'pending', source: 'dynamic_planning' });
    ...
    if (proposal.decision && project.state.mode === 'pause') { project.state.mode = 'run'; project.status.decisionPending = null; }
  ...
  function handle(input, { method = 'GET', body = {} } = {}) {
    ...
    if (method === 'GET' && path === '/projects/directories') return response({ ok: true, directories: [
    ...
    if (method === 'POST' && path === '/projects/open') {
      if (!['/mock/new-product', '/mock/cc-work', '/mock/unreadable'].includes(body.path)) return fail('请选择一个模拟目录', 400);
      if (!projects.has(body.path)) {
        const next = createProject(body.path, 0, body.path === '/mock/cc-work' ? 'idle' : 'directory');
        next.environment.status = 'reading'; next.environmentFailure = body.path === '/mock/unreadable'; projects.set(body.path, next);
      }
      return response({ ok: true, projectRoot: body.path }, 201);
    }
    if (!projects.size && method === 'GET' && path === '/status') return response({ ok: true, projects: [], session: false, state: 'idle' });
    ...
    if (method === 'GET') {
      ...
      if (path === '/run/status') {
        const runs = project.runs.map(run => ({ ...run, counts: run.status === 'running' ? taskCounts(project) : run.counts || taskCounts(project) }));
    ...
    if (method === 'POST') {
      if (path === '/run/state/mode') {
        ...
        if (body.mode !== 'idle' && !project.runs.some(r => r.status === 'running')) return fail('没有活动运行');
      ...
      if (path === '/run/submit') {
        ...
        if (project.requirements.length && project.plan.status !== 'approved') return fail('请先确认本轮计划');
        if (!project.state.tasks.some(t => t.status === 'pending')) return fail('没有可执行任务，请先确认计划');
        if (body.runId && project.runs.some(r => r.runId === body.runId)) return fail('Run ID 已存在');
        const run = { runId: body.runId || `run-${project.runs.length + 1}`, status: 'running', mode: body.mode || 'batch', startedAt: now() };
        project.runs = project.runs.filter(r => r.runId !== run.runId); project.runs.push(run); project.state.mode = 'run'; project.state.currentState = 'CODE';
      ...
      if (path === '/send' || path === '/respond') {
        ...
        if (path === '/respond' && project.status.decisionPending.kind === 'choice' && !project.status.decisionPending.options[Number(text) - 1]) return fail('请选择有效选项', 400);
        project.messages.push({ id: `M-${Date.now()}-${project.messages.length}`, role: 'user', type: 'text', text, at: now() });
      ...
      if (path === '/stop') {
        ...
        project.snapshot += '\n当前响应已打断。';
        project.messages.push({ id: `M-stop-${project.messages.length}`, role: 'assistant', type: 'text', text: '当前响应已打断，可以继续发送补充说明。', at: now() }); emit(project, 'conversation.interrupted', {});
      ...
      if (override) {
        ...
        if (project.decisions.some(d => d.decision_id === id && d.event === 'decision_overridden')) return fail('决策已调整');
      ...
      if (approval || resolution) {
        ...
        if (proposal.decision) project.decisions.push({ event: 'decision_completed', decision_id: proposal.decision.decisionId, runStamp: proposal.decision?.runStamp || 'mock-run', status: 'reviewed', subject: { capability: 'dynamic_planning', proposal_id: proposal.proposalId }, result: { answer: proposal.status === 'conflicted' ? '审批已记录，但变更冲突' : '批准并应用提案', type: 'resolved' } });
  ...
  function advance() {
    for (const project of projects.values()) {
      ...
      if (project.replyPending) { project.status.state = 'ready'; project.replyPending = false; project.snapshot += '\n✓ 本次回复已处理。'; project.messages.push({ id: `M-reply-${project.tick}`, role: 'assistant', type: 'text', text: project.runs.some(r => r.status === 'running') ? '已记录你的补充要求，继续按当前计划执行。' : '已记录你的补充要求。可以在项目页面添加下一条需求。', at: now() }); emit(project, 'conversation.replied', {}); }
      ...
      if (queued && project.state.mode === 'run') { queued.status = 'running'; emit(project, 'run.started', { mode: queued.mode }); }
      ...
      if (project.tick % 3 === 0) {
        ...
        if (active) { active.status = 'done'; project.messages.push({ id: `M-task-${project.tick}`, role: 'tool', type: 'task', title: active.title, text: '实现完成，验收检查通过。', status: 'done', at: now() }); emit(project, 'task.done', { taskId: active.id, title: active.title }); project.snapshot += `\n✓ ${active.id} ${active.title}`; }
        if (active && project.requirements.length && !project.proposals.length) {
          const target = project.state.tasks.find(t => t.status === 'pending');
          if (target) {
            const proposalId = `P-${project.requirements.length}-LIVE`, decisionId = `D-${project.requirements.length}-LIVE`;
            target.status = 'blocked'; project.state.mode = 'pause';
            project.proposals.push({ proposalId, status: 'decision_required', reason: '运行中发现需要补充接口契约验证', createdAt: now(), decision: { decisionId, runStamp: run.runId }, analysis: { affectedTaskIds: [target.id], decisionReasons: ['新增前置验证任务'] }, operations: [{ type: 'insert_task', task: { id: `CHECK-${project.requirements.length}`, title: '补充接口契约验证', kind: 'test' }, relation: { type: 'prerequisite_for', targetTaskId: target.id } }] });
            project.decisions.push({ event: 'decision_requested', decision_id: decisionId, runStamp: run.runId, status: 'awaiting_human', real_question: '是否加入接口契约验证，再继续执行？', subject: { capability: 'dynamic_planning', proposal_id: proposalId } });
            project.messages.push({ id: `M-${decisionId}`, role: 'assistant', type: 'decision', title: '需要你的决策', text: '发现缺少接口契约验证，已暂停。请到决策或动态复审页面批准变更，或提交其他方案。', status: 'awaiting_human', at: now() });
            emit(project, 'decision.requested', { decisionId, proposalId });
          }
        }
        const pending = project.state.mode === 'run' && project.state.tasks.find(t => t.status === 'pending' && (t.deps || []).every(id => project.state.tasks.some(dep => dep.id === id && dep.status === 'done')));
        ...
        if (!summary.pending && !summary.active && !summary.blocked && !project.state.tasks.some(t => t.status === 'failed')) { run.status = 'done'; run.counts = summary; run.endedAt = now(); project.state.mode = 'idle'; project.state.currentState = 'DONE'; emit(project, 'run.stopped', { status: 'done' }); }
      }
      if (project.state.tasks.some(t => ['blocked', 'failed'].includes(t.status)) && !project.state.tasks.some(t => t.status === 'active')) { project.state.mode = 'pause'; emit(project, 'run.blocked', {}); }
      project.snapshot = project.snapshot.slice(-12000);
    }
  }
  return { handle, advance, scenario, recover: () => { unavailable = false; }, subscribe: callback => { listeners.add(callback); return () => listeners.delete(callback); } };
}
