...
> **本文件是笔记（append-only 讨论记录），不是架构正文。**
> v0.2.0 的**正式架构终稿**（纪律文字化 / 组织图 / 功能映射 / 决策归档）见 [`architecture-v0.2.0.md`](./architecture-v0.2.0.md)；
> 门禁审查的纪律正文以该文件 §1 为准。早期形态（v0.1.x）已归档到 [`_archived/`](./_archived/)。
...
2. **方法论内置** — 插件内的命令与 skill 不只是工具，是一套可复现的开发方法论（721 测试金字塔、约定式提交、WBS 分解、Issue 升级机制）
...
## AI 主导全量重构的元实验（v0.2.0 复盘摘要，2026-09-11）

> 完整复盘：`docs/discuss/ai-refactor-retro-v0.2.0.md`（卡点 / 边界决策 / 验证结论 / Stable-Improve-Experiment / 行动项）

**一句话结论**：这一版最大的产出不是重构后的代码，而是**把教训变成了会失败的检查**。

**反复出现的单一失效模式**：把「登记/验证的形式成立」当成「事实成立」——
抽象建了 ⇒ 以为接上了；测试绿 ⇒ 以为落地了；写了责任人 ⇒ 以为有人做；契约写了 ⇒ 以为实现如此。
**每一次都是形式先成立、事实后缺席，而且缺席时不响。**

**四条从教训转成的判据**（本仓现行制度，改动前须先推翻）：

| 教训 | 判据 | 落点 |
|---|---|---|
| 抽象建了没人用 | 生产侧零引用 **且** 测试 import 不算采用 ⇒ 门禁红 | `check-architecture` 不变量① |
| 写了责任人 ≠ 责任人可达 | 依赖闭包内有 blocked ⇒ 该任务永不可就绪 ⇒ 报警 | `unreachableResponsibles()` |
| 失败被兜底藏起来 | 缺 `?p` 的写请求 → 400；缺产物 → 503 + 告警；缺依赖 → 构建中断 | server / build 脚本 |
| 自述把目标写成现状 | 只写能取证的（附 `文件:行号`）；契约声明须有存在性断言钉住 | `code-doc` 口径 + 契约单测 |

**关于人与 AI 的分工**（本次实验最有价值的一条经验）：
153 个任务、8 个模块门禁、1 次全量真机门禁全部由 AI 推进，**人只在产品取向上出现四次**
（决策入口换代、互斥化暂缓、`awf plan` 暂不测、A/D 里程碑口径），工程判断全部自决。
边界是：**「要什么」由人划，「怎么做」由 AI 自决**。
反例同样有信息量：锁文件去留最初被上抛给人，最终由**决策门阀自决** —— 仓政策类问题本就不该占用人。

**下轮复盘的第一节必须先核对本轮行动项 A1–A3**（分别归 `T1-120`、`awf-run-test`/`awf-plan-tasks` 技能、`T3-011-F1`）。
