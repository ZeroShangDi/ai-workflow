...
## 2026-09-11 追加：动态规划是首例**刻意双写**，两套的边界要在这里划清

动态任务规划能力落地时，**同一场景同时进了两套**（用户明确要求）：

| | 回归 | eval |
|---|---|---|
| case | `dynamic-planning`（31 断言，无模型）+ `dynamic-planning-run`（22 断言，真 tmux + 真 Claude，**一次 run 走到底**） | `cases/dynamic-planning/`（`case.json` + `hooks.mjs`） |
| 断言口径 | 链路不变量：proposal/hold/批准/重放/`conflicted`、顺序用 `startedAt` 判 | 产出质量：`files`/`tasksDone` + 钩子里的运行中事实 |
| 新增机制 | —— | **`hooks.mjs`**：`duringRun` / `afterRun`。声明式 `case.json` 只能「跑完看结果」，而这个能力的关键动作发生在 run **进行中**（人工批准一次 proposal），所以给用例开了与 run **并发**的钩子口 |

要点（进入本方案时不能丢）：

1. 这是**同一个能力的两套证据**，不是两个能力。合并方案落地时它们该收敛成**一份 case 定义 + 两个消费面**（本文件 §二），
   否则「一套更新、另一套过期」的老问题会立刻在这对 case 上重演。
2. 两套的**能力差异是真实的**：eval 侧多出「运行中钩子」这一维度，回归侧多出「不经模型的边界链路」。
   若最终只留一套，必须先把这两个维度都并进同一份 case 形态里。
3. 用户 2026-09-11 的裁定：**这两条线至少要能区分开**（职责、入口、产物、断言口径各自说清），
   合并/去重仍留待评审 —— 本条即为该裁定的登记。
4. **同日发现的硬证据：eval 这一套已经整体失效**（`.awf/issues/007`）。用例 prompt 仍写裸命令 `/w-dev`，
   而命令早已归属 `ai-workflow-code` 命名空间；用未改动的 `hello-sum` 对照实测，Claude 只回
   `Unknown command: /w-dev`，任务永不结算 → 用例挂到超时。**「一套跟着插件改名走了、另一套留在原地」
   正是本文件开头担心的形态，而且已经发生**：回归套件用 `/ai-workflow-code:w-dev` 一直正常。
   清扫前任何「eval 全绿」的说法都不成立。
