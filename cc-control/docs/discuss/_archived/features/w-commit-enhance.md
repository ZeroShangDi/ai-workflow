...
2. **设计新命令/技能时参考规范** — 后续扩展时，查阅 `sys-rule-workflow` skill 确定新功能应该做成 command 还是 skill
...
- **包含**：
  ...
  - 新增 `sys-rule-workflow` skill：定义 command 和 skill 的职责边界、规模约束、设计原则
...
- [ ] 新增 `sys-rule-workflow` skill，定义 commands（≤150行）和 skills（≤300行）的规范
