...
> 源码文件：`（已删除：能力停用，见 .awf/issues/017）`（**此前误写为 `（已废弃，见 .awf/issues/017）`，该文件不存在**）
> 测试文件：`tests/unit/version-prompt.test.js`

> **当前状态**：该功能在生产路径上**未接线**（`cli/commands/init.cjs:5`、`cli/commands/plan.cjs:2` 的 import/调用均被注释，注明「版本处理暂时禁用」）。
> 下方用例是 `（已删除：能力停用，见 .awf/issues/017）` 的**单元**行为，仍全部在册且通过；它们不证明该功能已接入 `awf init` / `awf plan`。
