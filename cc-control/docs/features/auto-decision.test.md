...
> 源码文件：`cli/lib/client.cjs`（autoSelect / waitForReady）+ `server/server.cjs`（`/hook`, `/choice`, `/ask`, `/respond`, `/status`）+ `server/run/transport.cjs`（多 agent NEEDS_INPUT 挂起）
...
### cli/lib/client.cjs（autoSelect / waitForReady）— 4 个 TC
