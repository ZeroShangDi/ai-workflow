...
> 来源：`plugin_old/skills/flow-rule-task/SKILL.md`
> 待合并目标：`awf-plan-tasks/SKILL.md`（粒度判定节）
> 状态：待用户审查，合并后删除本文件
>
> 与现版「三维复杂度判定」互补：本文件提供「独立可验证」首要标准、上下文修正器、非实现任务清单与实例。合并时注意现版编号/字段名（`T{级别}-{序号}`、`wbsRef`）已变。
