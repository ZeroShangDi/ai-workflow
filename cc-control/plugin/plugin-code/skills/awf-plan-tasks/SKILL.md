...
- 每份可独立生成、独立验证的文档是普通执行叶子：`T1-{序号}`、`kind=doc`，`plannedFiles` 写明目标文档；目标文件不冲突时可并行。
- `T4-{项目序号}` 只表示项目级文档完整性门禁：每个 `W4` 项目节点恰好一个，负责最终核对项目文档集合，不按文档数量重复创建。
- 禁止多个 `T4-*` 指向同一个 `W4-*`；例如三个文档应生成三个 T1 文档任务，再由一个 T4 门禁依赖它们。
...
```text
T1-006 生成需求 A 文档（kind=doc, plannedFiles=[docs/a.md]） ┐
T1-007 生成需求 B 文档（kind=doc, plannedFiles=[docs/b.md]） ├─ 可并行
T1-008 生成需求 C 文档（kind=doc, plannedFiles=[docs/c.md]） ┘
T4-001 项目文档完整性门禁（deps=[...,T1-006,T1-007,T1-008], wbsRef=W4-001）
```

编号即级别：执行叶子（含独立文档产出）=`T1`、审查门禁=`T2`、测试门禁=`T3`、项目文档门禁=`T4`，门禁序号与管辖节点对齐。生成顺序即后序：对话（共享）→ 缓存（共享）→ 意图识别（独立）→ 模块测试 → 文档产出 → 项目文档门禁。被依赖模块的任务先产出，独立模块在后。
