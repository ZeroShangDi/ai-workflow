---
description: 审查流程：对开发产出做多维度审查
argument-hint: <任务ID 或改动范围>
hint: <任务ID 或改动范围>
---
# w-review
...
4. 可维护性 — 命名、结构、重复代码
5. 架构 — 变化是否集中在正确边界，重构时机是否合理；并逐条对照架构纪律（依赖单向 / 实例隔离 / 事件 schema 版本）取证
...
- 架构纪律（依赖单向 / 实例隔离 / 事件 schema 版本）逐条给结论，每条附证据位置；未取证的纪律不得计入 pass
