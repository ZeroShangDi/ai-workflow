 ...
 * 断言会被夹具自身的不完整带偏（真实仓库里 cli/awf.cjs 是入口，白名单放行）。
 */
const BASE = { 'cli/awf.cjs': "require('./a.cjs');\n", 'cli/a.cjs': 'module.exports = 1;\n' };
const BASE_ALLOW = [{ file: 'cli/awf.cjs', reason: '入口（夹具）' }];
...
describe('不变量① 零生产引用模块', () => {
  it('生产侧零引用 → 检出，并指出文件路径', () => {
    const root = tree({
      'server/shared/used.cjs': 'module.exports = 1;\n',
      'server/shared/dead.cjs': 'module.exports = 2;\n',
      'cli/a.cjs': "require('../server/shared/used.cjs');\n",
    });
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} });
    expect(keysOf(r, 'zero-ref')).toEqual(['server/shared/dead.cjs']);
    expect(r.findings.find((f) => f.key === 'server/shared/dead.cjs').path).toBe('server/shared/dead.cjs');
  ...
  it('只被 tests/ 引用不算「被采用」——仍报', () => {
    const root = tree({
      'server/shared/dead.cjs': 'module.exports = 2;\n',
      'tests/unit/x.test.js': "require('../../server/shared/dead.cjs');\n",
    });
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} });
    expect(keysOf(r, 'zero-ref')).toEqual(['server/shared/dead.cjs']);
  });
  ...
  it('被 plugin/ 生产侧引用也算被采用', () => {
    const root = tree({
      'server/shared/used.cjs': 'module.exports = 1;\n',
      'plugin/core/mcp/x/server.cjs': "require('../../../../server/shared/used.cjs');\n",
    });
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} });
  ...
  it('计算路径 require 也算引用（字面扫描看不见它）', () => {
    const root = tree({
      'server/shared/store-core.cjs': 'module.exports = 1;\n',
      'plugin/core/mcp/x/server.cjs': "const p = require('node:path');\n"
        + "require(p.join(__dirname, '..', '..', '..', '..', 'server', 'shared', 'store-core.cjs'));\n",
    });
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} });
    expect(keysOf(r, 'zero-ref')).toEqual([]);
  });
  ...
  it('白名单逐文件放行（不靠 glob）', () => {
    ...
    expect(keysOf(runCheck({ root, allowlist: [], exemptions: {} }), 'zero-ref')).toEqual(['cli/awf.cjs']);
  });

  it('ESM 消费壳（同名 .cjs 存在）放行', () => {
    const root = tree({
      'server/shared/foo.cjs': 'module.exports = 1;\n',      // 真实实现（被引用）
      'server/shared/foo.js': "export { x } from './foo.cjs';\n", // 同名 ESM 壳（自身零引用）
      'cli/a.cjs': "require('../server/shared/foo.cjs');\n",
    });
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} });
    expect(keysOf(r, 'zero-ref')).toEqual([]);
  });
  ...
  it('没有同名 .cjs 的 .js 不放行（谓词是精确的，不是「只要是 .js 就放」）', () => {
    const root = tree({ 'server/shared/plain.js': 'export const x = 1;\n' });
    expect(keysOf(runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} }), 'zero-ref')).toEqual(['server/shared/plain.js']);
  });
});
 ...
 * 当场失效。实测被漏检的真身是 `server/shared/version.js`。以下四条把「剥注释」的正反两面都钉住。
 */
describe('不变量① 注释里的 import 不算引用（issue 003）', () => {
  it('整行 // 注释里的 import → 判零引用', () => {
    const root = tree({
      'server/shared/version.js': 'export const x = 1;\n',
      'cli/a.cjs': "// import { x } from '../server/shared/version.js'; // 暂时禁用\n",
    });
    expect(keysOf(runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} }), 'zero-ref'))
      .toEqual(['server/shared/version.js']);
  });

  it('块注释里的 import → 判零引用', () => {
    const root = tree({
      'server/shared/legacy.js': 'module.exports = 1;\n',
      'cli/a.cjs': "/*\n * require('../server/shared/legacy.js'); ← 已下线\n */\nmodule.exports = 1;\n",
    });
    expect(keysOf(runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} }), 'zero-ref'))
      .toEqual(['server/shared/legacy.js']);
  });

  it('真 import 与注释里的 import 并存 → 不算零引用（剥离不误伤真代码）', () => {
    const root = tree({
      'server/shared/used.cjs': 'module.exports = 1;\n',
      'cli/a.cjs': "require('../server/shared/used.cjs');\n// require('../server/shared/used.cjs');\n",
    });
  ...
  it('已知残留：**行尾**注释里的 import 仍被计为引用（只剥整行，避免误伤字符串里的 //）', () => {
    const root = tree({
      'server/shared/tail.js': 'module.exports = 1;\n',
      'cli/a.cjs': "const a = 1; // require('../server/shared/tail.js')\n",
    });
    expect(keysOf(runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} }), 'zero-ref')).toEqual([]);
  });
...
describe('不变量② 依赖方向', () => {
  it('越出 ALLOWED → 检出（key 为层边）', () => {
    // shared 是内层，只许向内：它反向依赖编排层 run 就是越界
    const root = tree({
      'server/shared/a.cjs': "require('../run/b.cjs');\n",
      'server/run/b.cjs': 'module.exports = 1;\n',
    });
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions: {} });
    expect(keysOf(r, 'dep-direction')).toEqual(['server-shared → server-run']);
    expect(r.findings.find((f) => f.check === 'dep-direction').detail).toContain('a.cjs');
  });

  it('声明方向内不报', () => {
    const root = tree({
      'cli/a.cjs': "require('../server/shared/b.cjs');\n",
      'server/shared/b.cjs': 'module.exports = 1;\n',
    });
  ...
  it('ALLOWED 未被为过门禁而放宽（守住 T1-114 的硬约束）', () => {
    expect([...ALLOWED].sort()).toEqual([
      'cli → server-adapters', 'cli → server-constants', 'cli → server-features', 'cli → server-shared',
      'plugin-mcp → server-adapters', 'plugin-mcp → server-shared',
      'scripts → server-shared',
      'server-adapters → server-constants', 'server-adapters → server-shared',
      'server-entry → server-observability', 'server-entry → server-runtime', 'server-entry → server-web',
      'server-features → server-adapters', 'server-features → server-constants',
      'server-features → server-observability', 'server-features → server-shared',
      'server-observability → server-adapters', 'server-observability → server-constants',
      'server-observability → server-shared',
      'server-run → server-adapters', 'server-run → server-constants', 'server-run → server-shared',
      'server-runtime → server-adapters', 'server-runtime → server-constants', 'server-runtime → server-features',
      'server-runtime → server-observability', 'server-runtime → server-run', 'server-runtime → server-shared',
      'server-shared → server-constants',
      'server-web → server-adapters', 'server-web → server-constants', 'server-web → server-observability',
      'server-web → server-runtime', 'server-web → server-shared',
    ]);
  });
});
...
describe('不变量③ 可配置结构断言', () => {
  ...
  it('maxLines 超限 → 检出（后续任务直接加一条规则即可）', () => {
    const root = tree({ 'server/shared/big.cjs': 'x\n'.repeat(20) });
    const rules = rule({ id: 'big', kind: 'maxLines', file: 'server/shared/big.cjs', max: 10, reason: '测试上限' });
  ...
  it('maxLines 未超 → 不报', () => {
    const root = tree({ 'server/shared/small.cjs': 'x\n' });
    const rules = rule({ id: 'small', kind: 'maxLines', file: 'server/shared/small.cjs', max: 10, reason: '测试上限' });
    expect(keysOf(runCheck({ root, allowlist: [], exemptions: {}, structureRules: rules }), 'structure')).toEqual([]);
  });

  it('forbidFiles 命中 → 检出（备给「server 不得放 html/css」这类目标结构）', () => {
    const root = tree({
      'server/legacy.html': '<html></html>',
      'server/ok.cjs': 'module.exports = 1;\n',
    });
    const rules = rule({ id: 'no-html', kind: 'forbidFiles', dir: 'server', extensions: ['.html', '.css'], reason: '静态资源归 web/' });
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions: {}, structureRules: rules });
    expect(keysOf(r, 'structure')).toEqual(['no-html']);
    expect(r.findings.find((f) => f.check === 'structure').path).toBe('server/legacy.html');
  });

  it('未知 kind → 显式抛错（不静默跳过规则）', () => {
    const root = tree({ 'server/shared/a.cjs': 'module.exports = 1;\n' });
...
describe('豁免表与 --strict', () => {
  it('豁免生效：finding 被标记，且不进 blocked', () => {
    const root = tree({ 'server/shared/dead.cjs': 'module.exports = 1;\n' });
    const exemptions = { 'server/shared/dead.cjs': { reason: '待接线', responsible: 'T1-115' } };
    const r = runCheck({ root, allowlist: BASE_ALLOW, exemptions });
    expect(r.blocked).toEqual([]);
    expect(r.exemptions).toEqual([{ key: 'server/shared/dead.cjs', check: 'zero-ref', reason: '待接线', responsible: 'T1-115' }]);
  });

  it('默认只报不拦（豁免不进 blocker）；--strict 下豁免表非空即失败', () => {
    const root = tree({ 'server/shared/dead.cjs': 'module.exports = 1;\n' });
    const exemptions = { 'server/shared/dead.cjs': { reason: 'r', responsible: 'T1-115' } };
  ...
  it('豁免不存在的项 → 不产生豁免记录（陈旧豁免可被 --strict 揪出）', () => {
    const root = tree({ 'server/shared/a.cjs': "require('./b.cjs');\n", 'server/shared/b.cjs': 'module.exports=1;\n' });
    const exemptions = { 'server/shared/gone.cjs': { reason: '陈旧', responsible: 'T1-115' } };
...
describe('CLI 行为（退出码 + 指出路径）', () => {
  ...
  it('干净树 → 0；有零引用模块 → 非 0 且输出文件路径', () => {
    ...
    const clean = tree({ 'cli/a.cjs': "require('../server/shared/b.cjs');\n", 'server/shared/b.cjs': 'module.exports=1;\n' });
    ...
    expect(r.out).toContain('server/shared/dead.cjs');
  });

  it('越界导入 → 非 0 且指出路径', () => {
    const dirty = tree({
      'server/shared/a.cjs': "require('../server/b.cjs');\n",
      'server/b.cjs': 'module.exports=1;\n',
    });
    const r = runCli(['--root', dirty]);
    expect(r.code).toBe(1);
    expect(r.out).toContain('server/shared/a.cjs');
  });

  it('--json 输出机器可读结果', () => {
    const dirty = tree({ 'server/shared/dead.cjs': 'module.exports=1;\n' });
    const r = runCli(['--root', dirty, '--json']);
    const parsed = JSON.parse(r.out);
    expect(parsed.blocked.map((f) => f.key)).toContain('server/shared/dead.cjs');
  });
