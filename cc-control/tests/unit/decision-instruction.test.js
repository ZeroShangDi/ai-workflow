...
import pluginRender from '../../server/shared/plugin-render.cjs';
...
/**
 * server 决策模式指令读取：定位 decision 插件（经自动发现的插件注册表解析，不写死目录）
...
describe('decision-instruction — 定位 decision 插件并读取决策指令', () => {
  it('目录经自动发现的 marketplace 解析，等于注册的 decision 插件 dir', () => {
    const cfg = pluginRender.readPluginConfig(REPO);
