'use strict';
/**
 * run/channel.cjs — 单 agent 的**收尾协商**（会话内协商的另一段「上下文压缩检查」已独立到 context/）。
 *
 ...
 * @returns {{ settleTask(taskId): Promise<'done'|'blocked'> }}
 */
...
module.exports = {
  ...
  MAX_SETTLE_ROUNDS, SETTLE_MAX_TOTAL_ROUNDS, SETTLE_MIN_TURN_BYTES,
};
