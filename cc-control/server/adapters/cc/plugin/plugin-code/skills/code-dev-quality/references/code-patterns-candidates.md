...
> 来源：`plugin_old/skills/code-patterns/candidates.md`
> 待合并目标：`code-dev-quality/SKILL.md`（及按主题拆分至 code-dev-rule / code-dev-security / code-dev-performance / code-dev-experience-vue）
> 状态：待用户审查，合并进对应 SKILL.md 后删除本文件
>
> 每一条都包含：**坏味道示例** → **正确做法** → **原因**。
> 主题分布（供拆分参考）：一~三 条件/异步/函数 → code-dev-rule；四~五 类型/null → code-dev-security；六~七 集合/字符串 → code-dev-rule；八~九 Vue/性能 → code-dev-experience-vue + code-dev-performance；十~十三 VueUse/Pinia/Vite/Element Plus 源码提炼 → code-dev-design + code-dev-experience-vue。
