'use strict';
/**
 * cli/lib/server-log.cjs — 常驻 server 的输出落盘（T1-112；随旧树退役迁入）
 *
 ...
 *
 * 迁入说明：新 CLI 初版把这段内联进了 `cli/lib/session.cjs`（只有 `openSync(path,'a')`），
 * **轮转随之丢失 → server.log 无限增长**。收口时按原样迁回，见
 * `docs/discuss/legacy-tree-retirement.md` §5.2。
 */
...
const SERVER_LOG_MAX_MB = (() => {
  const raw = Number(process.env.CC_SERVER_LOG_MAX_MB);
  return Number.isFinite(raw) ? raw : 5;
})();

/** server 输出日志路径（日志目录由 run-context 单源给出） */
function serverLogPath(logsDir) {
 ...
 */
function openServerLog(logPath, { maxBytes = SERVER_LOG_MAX_MB * 1024 * 1024 } = {}) {
...
module.exports = { SERVER_LOG_MAX_MB, serverLogPath, openServerLog };
